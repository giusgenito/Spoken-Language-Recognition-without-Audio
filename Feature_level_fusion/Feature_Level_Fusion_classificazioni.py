"""
Con questo codice vogliamo unire i dataset delle probabilità previste
dal layer denso con il classificatore GaussianGPC e GaussianGNB
ed inserirli in una batteria di classificatori per valutare l'accuratezza
Per farlo però vanno prima splittati gli individui per avere una buona classificazione

NB: Nel test set composto da 160 video, vi sono 4 individui per ogni lingua,
(tutte le lingue sono bilanciate), noi vogliamo splittare il test set in 2,
una parte viene utilizzata come training e una come test.
Per evitare che i dati del train e del test siano sbilanciati dividiamo come segue:
Il dataset di training sarà composto da 3 individui per ogni lingua, e il dataset di test da 1 individuo per ogni lingua
ottenendo una sorta di split 80-20 ma non casuale, bensì basata sugli individui
"""
...
classifiers = [
    ...
    MLPClassifier(max_iter=6000),
    AdaBoostClassifier(),
    GaussianNB(),
    QuadraticDiscriminantAnalysis(),
]

print("Accuratezza con GNB")
print("----------------------------------------------------------")
...
for modello in classifiers:
    ...
    df = pd.DataFrame(
        {'Classe': classes, 'Precision': precision, 'Recall': recall, 'F1-score': f1, 'Accuracy': class_accuracies})
