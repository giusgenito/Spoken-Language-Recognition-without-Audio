...
def prepare_test(test_df, dir):
    ...
    labels = []
    path_video = []
    y_lable = pd.DataFrame(columns = ["Nomi_video", "Etichette"])

    for i in range(num_samples):
        path_video.append(dir + "/" + video_name_paths[i])

        labels.append(num_less_zero(int(str(video_name_paths[i])[0])))
        nome_video = video_name_paths[i].replace(".avi", "")
        nome_video = nome_video.replace("_m", "")
        y_lable.loc[i] = (nome_video,labels[i])
    return path_video, labels, y_lable

path_train, labels_train = prepare_train(train_df, dir_train)
path_valid, labels_valid = prepare_train(valid_df, dir_valid)
path_test, labels_test, y_lable = prepare_test(test_df, dir_test)

y_lable.to_csv('C:/Users/CASALAB/Desktop/Gruppo_20_Favb/PreProcessing/Y_lable_test_con_nomi.csv', index=False)
...
def pre_processing_train(path_train, labels_train):
    ...
    np.save('C:/Users/CASALAB/Desktop/Gruppo_20_Favb/PreProcessing/Training_Set_NpArr.npy', x)
    np.save('C:/Users/CASALAB/Desktop/Gruppo_20_Favb/PreProcessing/Training_Set_Y.npy', y)
...
def pre_processing_validation(path_valid, labels_valid):
    ...
    np.save('C:/Users/CASALAB/Desktop/Gruppo_20_Favb/PreProcessing/Validation_Set_NpArr.npy', x)
    np.save('C:/Users/CASALAB/Desktop/Gruppo_20_Favb/PreProcessing/Validation_Set_Y.npy', y)
...
def pre_processing_test(path_test, labels_test):
    ...
    np.save('C:/Users/CASALAB/Desktop/Gruppo_20_Favb/PreProcessing/Test_Set_NpArr.npy', x)
    np.save('C:/Users/CASALAB/Desktop/Gruppo_20_Favb/PreProcessing/Test_Set_Y.npy', y)
